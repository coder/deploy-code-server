...
import { createDroplet, Droplet, DropletV4Network, getDroplet } from '../../lib/digitalOcean';
...
const handleErrorLog = (error: unknown) => {
  if(isPermissionError(error)) {
    ...
    console.log(chalk.red("You may have to delete the droplet manually on your Digital Ocean dashboard."))
  }
}
...
export const deployDigitalOceanAction = async () => {
  const { token } = await inquirer.prompt([
    { name: "token", message: "Your Digital Ocean token:" }
  ])
  ...
  try {
    ...
    let droplet = await createDroplet({ userData: await getUserDataScript(), token })
    spinner.stop()
    ...
    spinner = ora('Waiting code-server to be live. It can take up to 5 minutes.').start(); 
    await waitUntilCodeServerIsLive(droplet, token)
    droplet = await getDroplet({ token, id: droplet.id })
    spinner.stop()
    console.log(chalk.green(`🚀 Your code-server is live. You can access it on`, chalk.bold(`http://${getPublicIp(droplet)}`)))
  } catch(error) {
    spinner.stop()
